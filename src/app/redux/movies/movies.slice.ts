...
>('@@movies/putMovie', async ({ id, payload }, thunkApi) => {
    const response = await api.moviesService.putMovie(id, payload);
    return response;
});

export const createMovie = createAsyncThunk<any, IPutMovieDetails>(
    '@@movies/createMovie',
    async (payload) => {
...
export const getMoviesAdmin = createAsyncThunk<IMoviesGetAll, undefined>(
    '@@movies/getMoviesAdmin',
    async () => {
        const { data } = await api.moviesService.getAllAdminMovies();
        return data;
    },
);
const moviesSlice = createSlice({
    ...
    extraReducers(builder) {
        ...
        builder.addCase(getMovies.fulfilled, (state, action) => {
            ...
            state.showingNow = action.payload.data.filter(
                (m) => m.status.id === 2,
            );
            state.comingSoon = action.payload.data.filter(
                (m) => m.status.id === 1,
            );
        });
